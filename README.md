...
Token 存储在配置文件或环境变量中。**注意：** `openclaw config get gateway.auth.token` 会脱敏显示为 `__OPENCLAW_REDACTED__`，无法直接复制。要填入本应用请从配置文件读取：

```bash
# 推荐：从配置文件读取真实 Token（用于复制到本应用配置）
